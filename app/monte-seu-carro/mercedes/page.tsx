...
function ConteudoMonteSeuCarro() {
  ...
  // Listas de materiais separadas por peça
  const [paintMaterials, setPaintMaterials] = useState<any[]>([]); 
  const [interiorMaterials, setInteriorMaterials] = useState<any[]>([]); 
  const [parteBaixoMaterials, setParteBaixoMaterials] = useState<any[]>([]); 
  const [parabrisaLateralMaterials, setParabrisaLateralMaterials] = useState<any[]>([]); 
  ...
  // Estados de cor em formato HEX padrão
  const [selectedColor, setSelectedColor] = useState("#FF6600");
  const [selectedInteriorColor, setSelectedInteriorColor] = useState("#B85D19");
  const [selectedParteBaixoColor, setSelectedParteBaixoColor] = useState("#080808");
  const [selectedParabrisaLateralColor, setSelectedParabrisaLateralColor] = useState("#111111");
  const [selectedWheelColor, setSelectedWheelColor] = useState("#2E3033");
  const [selectedTaillightColor, setSelectedTaillightColor] = useState("#D90429");
  ...
  const resetDebugColors = () => {
    ...
    applyMaterialColor(parteBaixoMaterials, selectedParteBaixoColor);
    applyMaterialColor(parabrisaLateralMaterials, selectedParabrisaLateralColor);
          ...
          api.addEventListener('viewerready', () => {
            ...
            api.getMaterialList((err: any, materials: any[]) => {
              if (!err && materials) {
                ...
                const pinturas: any[] = [];
                const interior: any[] = [];
                const parteBaixo: any[] = [];
                const parabrisaLateral: any[] = [];
                const rodas: any[] = [];
                ...
                materials.forEach(m => {
                  ...
                  if (n === "interior" || n.includes("interior")) {
                    interior.push(m);
                  } else if (n === "black" || n.includes("black") || n === "plas_r" || n.includes("plas_r")) {
                    parteBaixo.push(m);
                  } else if (n === "plasgloss" || n.includes("plasgloss")) {
                    parabrisaLateral.push(m);
                  } else if (n.includes("wheel") || n.includes("tire") || n.includes("rim")) {
                    rodas.push(m);
                  } else if (n.includes("tail") || n.includes("light") || n.includes("lamp")) {
                    luzTraseira.push(m);
                  } else if (n.includes("glass") || n.includes("window") || n.includes("vidro") || n.includes("windshield") || n.includes("logo") || n.includes("emblem")) {
                    return;
                  } else {
                    pinturas.push(m);
                  }
                ...
                setParteBaixoMaterials(parteBaixo);
                setParabrisaLateralMaterials(parabrisaLateral);
  ...
  useEffect(() => { applyMaterialColor(parteBaixoMaterials, selectedParteBaixoColor); }, [selectedParteBaixoColor, sketchfabApi, parteBaixoMaterials]);
  useEffect(() => { applyMaterialColor(parabrisaLateralMaterials, selectedParabrisaLateralColor); }, [selectedParabrisaLateralColor, sketchfabApi, parabrisaLateralMaterials]);
        ...
        <div className="flex flex-col gap-6 pt-16 md:pt-4">
          <div className="flex items-center justify-between border-b border-white/10 pb-4">
            <h2 className="text-sm font-bold tracking-widest text-purple-400 uppercase flex items-center gap-2">
              <Settings size={18} /> Configurador de Veículo
            </h2>
          ...
          {/* 1. Pintura Externa Principal (Carroceria) */}
          ...
          {/* Parte de Baixo (Black e plas_R) */}
          <div className="flex flex-col gap-2">
            <span className="text-xs text-purple-400 uppercase font-bold flex items-center gap-1.5">
              <Layers size={14} /> 3. Parte de Baixo 
            </span>
            <ColorPickerCustom selectedColorHex={selectedParteBaixoColor} onChangeColor={setSelectedParteBaixoColor} preco={pricingRules.pecaPequena} />
          </div>

          {/* Parabrisa e Laterais do Vidro (plasGloss) */}
          <div className="flex flex-col gap-2">
            <span className="text-xs text-purple-400 uppercase font-bold flex items-center gap-1.5">
              <Shield size={14} /> 4. Parabrisa e Laterais do Vidro 
            </span>
            <ColorPickerCustom selectedColorHex={selectedParabrisaLateralColor} onChangeColor={setSelectedParabrisaLateralColor} preco={pricingRules.pecaPequena} />
          </div>

          {/* Estilo dos Vidros */}
          <div className="flex flex-col gap-3 bg-[#12121a] p-4 rounded-2xl border border-white/10">
            <span className="text-xs text-purple-400 uppercase font-bold flex items-center gap-1.5">
              <Droplets size={14} /> Transparência dos Vidros
            </span>
            <div className="grid grid-cols-2 gap-3">
              {[
